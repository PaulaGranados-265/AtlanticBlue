...
app.get("/usuarios", (req, res) => {
  db.query("SELECT id_usuario, nombre_apellidos as nombre, rol FROM usuario", (err, results) => {
    if (err) {
...
app.post("/factura", (req, res) => {
  ...
  const checkPedidoSql = "SELECT id_pedido FROM pedido WHERE id_pedido = ?"
  db.query(checkPedidoSql, [id_pedido], (err, results) => {
    if (err) return res.status(500).json({ error: "Error en el servidor" })
    if (results.length === 0) return res.status(404).json({ error: "Pedido no encontrado" })

    // Verificar si ya existe factura
    const checkFacturaSql = "SELECT id_factura FROM factura WHERE id_pedido = ?"
    db.query(checkFacturaSql, [id_pedido], (err, facturaResults) => {
      if (err) return res.status(500).json({ error: "Error en el servidor" })
      if (facturaResults.length > 0) return res.status(400).json({ error: "Ya existe una factura para este pedido" })

      // Calcular total
      const calcularTotalSql = `
        SELECT 
          COALESCE(SUM(dp.cantidad * pl.precio), 0) + COALESCE(SUM(dbp.cantidad * b.precio), 0) AS total
        FROM pedido p
        LEFT JOIN detalle_pedido dp ON p.id_pedido = dp.id_pedido
        LEFT JOIN platos pl ON dp.id_plato = pl.id_plato
        LEFT JOIN detalle_bebida_pedido dbp ON p.id_pedido = dbp.id_pedido
        ...
        WHERE p.id_pedido = ?
      `
      db.query(calcularTotalSql, [id_pedido], (err, totalResults) => {
        if (err) return res.status(500).json({ error: "Error calculando total" })

        const total = totalResults[0].total || 0

        // CORREGIDO: usar `hora` en lugar de `fecha_factura`
        const insertFacturaSql = `
  INSERT INTO factura (id_pedido, total, hora, pagada) 
  VALUES (?, ?, CURTIME(), 0)
`
        db.query(insertFacturaSql, [id_pedido, total], (err, result) => {
          if (err) {
            console.error("❌ Error creando factura:", err.sqlMessage)
